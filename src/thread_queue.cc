...
Threading::SerialQueue::SerialQueue(const std::string &name): _name(name) , _running(true) {
    startLoop();
}
...
Threading::ConcurrentQueue::ConcurrentQueue(const std::string &name) : _name(name), _running(true) {
    startLoop();
}
...
void Threading::ConcurrentQueue::startLoop() {
    for(int i = 0, count = std::max(4, Threading::processorCount()); i < count; ++i) {
        Threading::dispatchOnNewThread([this]{
           while(running()) {
               ThreadClosure block;
               {
...
std::shared_ptr<ThreadQueue> Threading::getQueue(const std::string &name, QueueType type)
{
    queuesLock.lock();
