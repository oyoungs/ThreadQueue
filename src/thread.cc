...
int my_pthread_cond_timedwait_relative_np(pthread_cond_t *cond, pthread_mutex_t *mutex, const timespec *tmspec)
{
    timespec timeout;
    timeval time;
...
void Threading::dispatchOnNewThread(const ThreadClosure &closure)
{
    ...
    pthread_create(&thrSlf, &attr, (ThreadFunction)&IsThisRequired::run, holdObject);
